...
class OpMemoryFormatConverter(object):
    ...
    def convert(self, custom_code, fun_config):
        if "interface" in fun_config and (
            accepted_interface == "ALL"
            or (fun_config["interface"] in accepted_interface)
        ):
    ...
    def do_convert(self, custom_code, fun_config):
        ...
        placeholder_3d_pattern = (
            "\$\{PREFERRED_MEMORY_FORMAT_PLACEHOLDER_3D:-(?P<default>.*)\}"
        )
        # match string like "${PREFERRED_MEMORY_FORMAT_PLACHOLDER:-<default>}"
        placeholder_pattern = (
            "\$\{PREFERRED_MEMORY_FORMAT_PLACEHOLDER:-(?P<default>.*)\}"
        )
