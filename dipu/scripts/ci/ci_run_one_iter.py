...
def process_one_iter(log_file, clear_log, model_info: dict) -> None:
    ...
    if device == 'cuda':
        ...
        if (p2 == "stable_diffusion/stable-diffusion_ddim_denoisingunet_infer.py"):
            ...
            cmd_run_one_iter = f"""srun --job-name={job_name} bash -c "cd {parent_directory} && source scripts/ci/ci_one_iter.sh export_pythonpath_cuda {current_path} && source /mnt/cache/share/deeplinkci/github/dipu_env && cd mmlab_pack && source environment_exported && python {current_path}/{train_path}" """
            cmd_cp_one_iter = ""
        else:
