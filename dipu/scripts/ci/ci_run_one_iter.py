...
def process_one_iter(log_file, clear_log, model_info: dict) -> None:
    ...
    elif "trans" in p1 or "lora" in p1:
        train_path = p1 + "/" + p2
        config_path = ""
        work_dir = ""
        opt_arg = ""
    ...
    logging.info(
        f"train_path = {train_path}, config_path = {config_path}, work_dir = {work_dir}, opt_arg = {opt_arg}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "selection_of_model_list",
        type=str,
        nargs="?",
        default="traditional",
        choices=["traditional", "llm"],
        help="the selection of model list",
    )
    ...
    with open(yamlPath, "r", encoding="utf-8") as f:
        ...
        logging.info(
            f"model nums: {len(original_list)}, chosen model num: {selected_model_num}"
        )
