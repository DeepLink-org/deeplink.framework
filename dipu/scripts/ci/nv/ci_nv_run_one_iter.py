...
def run_cmd(cmd):
    ...
    if cp.returncode != 0:
        error = "Some thing wrong has happened when running command [{}]:{}".format(
            cmd, cp.stderr
        )
...
def get_gpu_info():
    ...
    for i in range(device_count):
        ...
        gpu_info.append(
            {"gpu_id": i, "total_memory": total_memory, "free_memory": free_memory}
        )
...
def process_one_iter(q, model_info):
    ...
    print(
        "train_path = {}, config_path = {}, work_dir = {}, opt_arg = {}".format(
            train_path, config_path, work_dir, opt_arg
        ),
        flush=True,
    )
    ...
    print(
        "cardnum:{},model:{},cur_card_free:{}".format(available_card, p2, cur_gpu_free),
        flush=True,
    )

    if p2 == "configs/stable_diffusion/stable-diffusion_ddim_denoisingunet_infer.py":
        cmd_run_cp = "CUDA_VISIBLE_DEVICES={} sh mmagic/configs/stable_diffusion/stable-diffusion_ddim_denoisingunet_one_iter.sh".format(
            available_card
        )
        run_cmd(cmd_run_cp)
    else:
        cmd_run_one_iter = "CUDA_VISIBLE_DEVICES={} sh SMART/tools/one_iter_tool/run_one_iter.sh {} {} {} {}".format(
            available_card, train_path, config_path, work_dir, opt_arg
        )
        cmd_cp_one_iter = "CUDA_VISIBLE_DEVICES={} sh SMART/tools/one_iter_tool/compare_one_iter.sh".format(
            available_card
        )
    ...
    print(
        "The running time of {} :{} hours {} mins {} secs".format(
...
if __name__ == "__main__":
    ...
    print(
        "model num:{}, chosen model num:{}".format(length, random_model_num), flush=True
    )
