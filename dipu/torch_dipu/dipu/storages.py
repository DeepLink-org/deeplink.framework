...
def __validate_dipu_device(location):
    ...
    if not is_available():
        raise RuntimeError(
            "Attempting to deserialize object on a DIPU "
    ...
    if device_idx >= cnt:
        raise RuntimeError(
            "Attempting to deserialize object on DIPU device "
...
def _dipu_deserialize(obj, location):
    if (
        location.startswith(__diputype__)
        or location.startswith(__dipu__)
        or (mockcuda and location.startswith("cuda"))
    ):
...
def _dipu_tag(obj):
    if obj.device.type == __diputype__:
        return f"{__diputype__}:{obj.device.index}"
...
UntypedStorage.__new__ = GetDeviceProxy(
    torch.UntypedStorage.__new__, pos=-1, caller="class_new"
)
