...
inline bool isDiffStrides(const IntArrayRef stride1,
                          const IntArrayRef stride2) {
...
inline bool canDirectCopy(const at::Tensor& dst, const at::Tensor& src) {
  ...
  int64_t srcBytes = src.unsafeGetTensorImpl()->unsafe_storage().nbytes();
  int64_t dstBytes = dst.unsafeGetTensorImpl()->unsafe_storage().nbytes();
  if (srcBytes != dstBytes || dst.numel() != src.numel() ||
      dst.options().dtype() != src.options().dtype()) {
    return false;
  }
  if (isDiffStrides(dst.strides(), src.strides())) {
    return false;
  }
  // view(with no-zero offset) direct copy may cause err(not sure how long real
  // stor data should be copyed) not supported
  if (dst.storage_offset() != 0 || src.storage_offset() != 0) {
    return false;
  }
  // even tensors have zero offset and same stride/type cannot do simple safe
  // direct copy because we cannot simply decide how much data will be copyed
  // from raw stor (unless check stride). so we always return false now. need
  // enhance in future, because always copy with the help of cpu is toooo0 slow.
  // **** check if copy safely using tensor.nbytes() when is_contiguous() =
  // true.
  return false;
}
...
at::Tensor& DIPUATenFunctions::copy_(at::Tensor& self, const at::Tensor& src,
  ...
  if (!canDirectCopy(self, src)) {
    ...
    if (dipu::isDeviceTensor(self)) {
      at::Tensor dst_cpu =
          at::empty_strided(self.sizes(), self.strides(),
