...
at::Tensor create_out(at::IntArrayRef sizes, at::IntArrayRef strides,
  ...
  if (strides.empty()) {
    return dipu::native::dipu_aten::empty(
        sizes, options.dtype_opt()->toScalarType(), options.layout_opt(),
        options.device_opt(), options.pinned_memory_opt(),
        options.memory_format_opt());
  } else {
    return dipu::native::dipu_aten::empty_strided(
        sizes, strides, options.dtype_opt()->toScalarType(),
...
struct Infer_add_Tensor final : public at::meta::structured_add_Tensor {
  ...
  void set_output_strided(int64_t output_idx, at::IntArrayRef sizes,
                          at::IntArrayRef strides, c10::TensorOptions options,
    ...
    at::meta::structured_add_Tensor::set_output_raw_strided(
  ...
  const at::Tensor& maybe_get_output() { return maybe_get_output(0); }
