...
static at::Tensor& custom_fallback_dipu_index_tensor_out(
    ...
    at::Tensor& out) {
    DIPU_REGISTER_LOG("custom fallback to cpu, name=index.Tensor_out"
                      << std::endl);
...
static at::Tensor& custom_fallback_dipu__index_put_impl_(
    ...
    DIPU_REGISTER_LOG("custom fallback to cpu, name=_index_put_impl_"
                      << std::endl);

    auto indices_cpu = to_cpu(indices);
    at::Tensor self_cpu = self.cpu();
    at::native::_index_put_impl_(self_cpu, indices_cpu, values.cpu(),
                                 accumulate, unsafe);
    self.copy_(self_cpu);
...
custom_fallback_dipu_native_batch_norm_out(
    ...
    at::native_batch_norm_out(out_cpu, save_mean_cpu, save_invstd_cpu,
                              input_cpu, weight_cpu, bias_cpu, running_mean_cpu,
...
custom_fallback_dipu_native_batch_norm(
    ...
    if (running_mean_opt.has_value() && running_mean_opt.value().defined()) {
        save_mean =
            at::empty(running_mean_opt.value().sizes(),
...
custom_fallback_dipu_linear_backward(const at::Tensor& input,
    ...
    if (output_mask[1]) {
        auto grad_weight_cpu = at::matmul(
            input_cpu.transpose(dims - 2, dims - 1), grad_output_cpu);
...
custom_fallback_dipu_native_batch_norm_backward(
    ...
    if (weight_opt.has_value() && weight_opt.value().defined()) {
        grad_weight = at::empty(
            weight_opt.value().sizes(),
