...
#define SUCCL_CALL(Expr)                                          \
  {                                                               \
    ...
    if (ret != succlSuccess) {                                    \
      std::cout << "call a succl function (" << #Expr             \
                << ") failed. return code= " << ret << std::endl; \
...
diclResult_t diclCommDestroy(diclComm_t comm) {
  SUCCL_CALL(succlCommDestroy(comm));
};
...
static bool toSucclDataType(at::ScalarType type, succlDataType_t &out) {
  static std::map<at::ScalarType, succlDataType_t> succlDataType = {
      ...
      {at::kInt, succlInt32},   {at::kLong, succlInt32},
      {at::kBool, succlUint8},  {at::kBFloat16, succlBfloat16},
  };
...
diclResult_t diclAllReduce(const void *sendbuff, void *recvbuff, size_t count,
  ...
  SUCCL_CALL(succlAllReduce(sendbuff, recvbuff, count, suDataType, suOp, comm,
                            stream));
}
...
diclResult_t diclBroadcast(const void *sendbuff, void *recvbuff, size_t count,
  ...
  ConvertScalarType(datatype);
  SUCCL_CALL(succlBroadcast(sendbuff, recvbuff, count, suDataType, root, comm,
                            stream));
}
...
diclResult_t diclReduce(const void *sendbuff, void *recvbuff, size_t count,
  ...
  ConvertScalarType(datatype);
  ConvertOpType(reduceOp);
  SUCCL_CALL(succlReduce(sendbuff, recvbuff, count, suDataType, suOp, root,
                         comm, stream));
}
...
diclResult_t diclReduceScatter(void *sendBuf, void *recvBuf, size_t recvCount,
  ...
  ConvertScalarType(dataType);
  ConvertOpType(op);
  SUCCL_CALL(succlReduceScatter(sendBuf, recvBuf, recvCount, suDataType, suOp,
                                comm, stream));
}
