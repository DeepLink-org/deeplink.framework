...
#define SUPA_CALL(Expr)                                                         \
  {                                                                             \
    ...
    if (__ret != suSuccess) {                                                   \
      printf("call a supa function (%s) failed. return code=%d", #Expr, __ret); \
    }                                                                           \
  }
...
DIPU_API DIPUDeviceProperties getDeviceProperties(int32_t device_index) {
  ...
  return {prop.name, prop.totalGlobalMem, prop.major, prop.minor, prop.multiProcessorCount};
}
...
DIPU_API void getDriverVersion(int *version) { SUPA_CALL(suDriverGetVersion(version)); }

DIPU_API void getRuntimeVersion(int *version) { SUPA_CALL(suRuntimeGetVersion(version)); }
...
DIPU_API void destroyStream(deviceStream_t stream) { SUPA_CALL(suStreamDestroy(stream)); }
...
DIPU_API void syncStream(deviceStream_t stream) { SUPA_CALL(suStreamSynchronize(stream)); }

DIPU_API bool streamNotNull(deviceStream_t stream) { return true; }

DIPU_API void streamWaitEvent(deviceStream_t stream, deviceEvent_t event) { SUPA_CALL(suStreamWaitEvent(stream, event)); }
...
DIPU_API void createEvent(deviceEvent_t *event) { SUPA_CALL(suEventCreate(event)); }

DIPU_API void destroyEvent(deviceEvent_t event) { SUPA_CALL(suEventDestroy(event)); }

DIPU_API void waitEvent(deviceEvent_t event) { SUPA_CALL(suEventSynchronize(event)); }

DIPU_API void recordEvent(deviceEvent_t event, deviceStream_t stream) { SUPA_CALL(suEventRecord(event, stream)); }
...
DIPU_API void memSetAsync(const deviceStream_t stream, void *ptr, int val, size_t size) { SUPA_CALL(suMemsetAsync(ptr, val, size, stream)); }
...
DIPU_API void memCopyD2DAsync(const deviceStream_t stream, size_t nbytes, deviceId_t dstDevId, void *dst, deviceId_t srcDevId, const void *src) {
