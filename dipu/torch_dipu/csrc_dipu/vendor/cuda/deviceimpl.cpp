...
void resetDevice(deviceId_t devId) { DIPU_CALLCUDA(::cudaDeviceReset()) }

void syncDevice() { DIPU_CALLCUDA(::cudaDeviceSynchronize()) }

// check last launch succ or not, throw if fail
void checkLastError() { DIPU_CALLCUDA(::cudaGetLastError()) }
...
void createEvent(deviceEvent_t* event) {
  ...
  DIPU_CALLCUDA(::cudaEventCreateWithFlags(
      event, enableTiming ? cudaEventDefault : cudaEventDisableTiming))
}
...
void memCopyD2DAsync(const deviceStream_t stream, size_t nbytes,
  ...
  if (dstDevId == srcDevId) {
    ...
    DIPU_CALLCUDA(
        ::cudaMemcpyPeerAsync(dst, dstDevId, src, srcDevId, nbytes, stream))
  }
}
