...
extern "C" aclError aclprofSetStampCallStack(void *stamp, const char *callStack, uint32_t len);
...
public:
  ...
  void enableProfiler(const std::string &dump_path, bool call_stack, bool record_shapes, bool profile_memory);
...
private:
  ...
  void recordCallStack(void* stamp, int64_t sequence_num, at::RecordScope scope);
...
void AscendProfiler::enableProfiler(const std::string &dump_path, bool call_stack, bool record_shapes, bool profile_memory) {
  if (enable_) {
...
void AscendProfiler::recordCallStack(void* stamp, int64_t sequence_num, at::RecordScope scope) {
  ...
  if (scope != at::RecordScope::BACKWARD_FUNCTION) {
    ...
    if (cs.empty()) {
      cs = torch::profiler::impl::prepareCallstack(torch::jit::tracer::pythonCallstack());
    }
  ...
  if (!call_stack_data.empty()) {
    DIPU_CALLACLRT(aclprofSetStampCallStack(stamp, call_stack_data.c_str(), call_stack_data.size()));
  }
}
...
void enableProfiler(const std::string &dump_path, bool call_stack, bool record_shapes, bool profile_memory) {
  AscendProfiler::instance().enableProfiler(dump_path, call_stack, record_shapes, profile_memory);
}
