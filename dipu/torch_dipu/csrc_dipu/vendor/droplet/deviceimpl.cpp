...
void syncDevice() { DIPU_CALLDROPLET(::tangDeviceSynchronize()) }

// check last launch succ or not, throw if fail
void checkLastError() { DIPU_CALLDROPLET(::tangGetLastError()) }
...
void memCopyD2D(size_t nbytes, deviceId_t dstDevId, void* dst,
  ...
  if (dstDevId == srcDevId) {
    ...
    throw std::runtime_error(
        "dipu device error with tangMemcpyPeer not supported");
  }
}
