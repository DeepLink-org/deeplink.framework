...
inline void* getCommPcclFuncAddr(const char* apiName) {
  ...
  static void* pcclDependHandler =
      dlopen(pcclLibDependName, RTLD_LAZY | RTLD_GLOBAL);
  if (pcclDependHandler == nullptr) {
    throw std::runtime_error(
        "Error: Failed to load libpccl.so. The required library "
        "'libtangrt_shared.so' is missing.\n"
        "Please ensure that 'libtangrt_shared.so' is installed and its path is "
