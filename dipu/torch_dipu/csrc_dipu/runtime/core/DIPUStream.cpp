...
DIPUStream getDIPUStreamFromPool(c10::DeviceIndex device) {
  device = initDIPUGlobal(device);
  // Initializes the stream pools (once)
  streamDeviceList[device]->initPool();
  return streamDeviceList[device]->getDIPUStreamfromPool();
}
