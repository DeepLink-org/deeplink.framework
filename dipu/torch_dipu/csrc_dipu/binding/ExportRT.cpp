...
void exportCommunicator(py::module& m) {
  ...
  py::class_<ProcessGroupDICL::WorkDICL,
      ...
      .def(py::init([](std::vector<std::shared_ptr<DICLComm>>& comms,
                       bool blockingWait, std::chrono::milliseconds opTimeout) {
             return ProcessGroupDICL::WorkDICL(comms, blockingWait, opTimeout);
           }),
           py::arg("comms"), py::arg("blockingWait"), py::arg("opTimeout"),
           py::call_guard<py::gil_scoped_release>())
      .def("_get_duration", &ProcessGroupDICL::WorkDICL::getDuration,
           py::call_guard<py::gil_scoped_release>(),
           R"(
              Returns:
