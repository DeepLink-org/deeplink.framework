...
function run_dipu_tests {
  ...
  run_test "${PYTORCH_TEST_DIR}/test/test_tensor_creation_ops.py" "$@" -v -f TestTensorCreationDIPU # --locals -f
  ...
  run_test "${PYTORCH_TEST_DIR}/test/test_reductions.py" "$@" -v -f TestReductionsDIPU
