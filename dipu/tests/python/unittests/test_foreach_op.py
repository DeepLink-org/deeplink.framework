...
class TestForeach(TestCase):
    ...
    def test_foreach_addcmul__scalar_list(self):
        ...
        torch._foreach_addcmul_(
            self=inputs_cpu, tensor1=tensor1s_cpu, tensor2=tensor2s_cpu, scalars=scalars
        )
        torch._foreach_addcmul_(
            self=inputs_dipu,
            tensor1=tensor1s_dipu,
            tensor2=tensor2s_dipu,
            scalars=scalars,
        )
        for i in range(len(inputs_cpu)):
            self.assertTrue(torch.allclose(
    ...
    def test_foreach_addcmul__scalar(self):
        ...
        value = torch.randn(1).item()
        torch._foreach_addcmul_(
            self=inputs_cpu, tensor1=tensor1s_cpu, tensor2=tensor2s_cpu, value=value
        )
        torch._foreach_addcmul_(
    ...
    def test_foreach_addcmul__tensor(self):
        ...
        torch._foreach_addcmul_(
            self=inputs_cpu, tensor1=tensor1s_cpu, tensor2=tensor2s_cpu, scalars=tensors
        )
        torch._foreach_addcmul_(
            self=inputs_dipu,
            tensor1=tensor1s_dipu,
