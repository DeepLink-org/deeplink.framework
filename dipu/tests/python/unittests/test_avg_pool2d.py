...
class TestAvgPool2d(TestCase):
    ...
    def test_avg_pool2d(self):
        ...
        dipu_y = torch.nn.functional.avg_pool2d(dipu_x, kernel_size=kernel_size, stride=stride)
        dipu_grady = torch.randn_like(dipu_y)
        dipu_y.backward(dipu_grady)

        cpu_x = x.requires_grad_(True)
        cpu_y = torch.nn.functional.avg_pool2d(cpu_x, kernel_size=kernel_size, stride=stride)
