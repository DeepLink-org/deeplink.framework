...
class TestAdaptiveAvgPool2dBackward(TestCase):
    def _cpu_op_exec(self, input_x, input_grad):
    ...
    def _dipu_op_exec(self, input_x, input_grad):
        input_x.requires_grad_(True)
        m = torch.nn.AdaptiveAvgPool2d(input_grad)
        output = m(input_x)
        output.backward(output)
    ...
    def test_adaptiveAvgPool2d_backward_1(self, device=dipu):
        cpu_input = torch.randn((1, 64, 8, 9), dtype=torch.float32)
        dipu_input = cpu_input.to(device)
        output_size = np.array((2, 3))
        cpu_output = self._cpu_op_exec(cpu_input, output_size)
    ...
    def test_adaptiveAvgPool2d_backward_2(self, device=dipu):
    ...
    def test_adaptiveAvgPool2d_backward_fp16(self, device=dipu):
