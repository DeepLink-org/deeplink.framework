...
#define DICP_ASCEND_CHECK_NULLPTR_ABORT(ptr) \
  DICP_CHECK_ABORT(ptr, "Variable is nullptr, please check.")
...
#define CALL_FUNC(Expr)                                       \
  do {                                                        \
    auto ret = Expr;                                          \
    if (ret != SUCCESS) {                                     \
      TRACK_GE(#Expr);                                        \
      throw std::runtime_error("dicp call function failed."); \
    }                                                         \
  } while (0);
