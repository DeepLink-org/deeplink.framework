...
class AtenToAscendTransformer(SingleOpTransformer):
    ...
    def split(self, x, split_size, dim=0):
        ...
        num_split = len(fx_traceback.get_current_meta()['val'])
    ...
    def prompt_attention_inference(self, q, k, v, seqlen, num_head, head_dim, num_key_value_heads):
        ...
        if isinstance(num_key_value_heads, torch.fx.proxy.Proxy):
            num_key_value_heads = int(sympy.N(num_key_value_heads.node.meta['val']))
        q_shape = list(q.node.meta['val'].shape)
        q_dtype = q.node.meta['val'].dtype
        seq_len = q_shape[1]
        ...
        fa = self.get_proxy(ascend_op.PromptFlashAttention, (q, k, v, num_head, seqlen, mask, head_dim, num_key_value_heads))
    ...
    def flash_attention_inference(self, q, all_k, all_v, current_lens, max_len, kvhead=-1, head=-1, dim=-1):
        ...
        for i in range(batch):
            ...
            # split path for dynamic & static 7B
            if len(self.sym_in_args) > 0 or len(self.sym_to_inputs) > 0:
