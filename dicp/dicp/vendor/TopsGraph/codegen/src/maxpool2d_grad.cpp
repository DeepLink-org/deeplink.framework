...
builder::Op enflame::MaxPool2D(std::shared_ptr<builder::Builder> tmp_builder,
                               builder::Op input, std::vector<long> ksize,
                               std::vector<long> strides,
                               std::vector<long> padding,
                               std::vector<long> shape) {
  auto res1 = builder::MaxPool2D(input, ksize, false, false, "NOTSET", "NCHW",
                                 strides, padding, {});
  ...
  auto res2 = builder::Const(tmp_builder, static_cast<void*>(pool_data.data()),
                             pool_type);
...
builder::Op enflame::MaxPool2D_Grad(
    std::shared_ptr<builder::Builder> tmp_builder, builder::Op out_grad,
    builder::Op in, std::vector<int> ksize, std::vector<int> strides,
  ...
  if (true) {
    ...
    if (do_transpose) {
      ...
      auto tmp_op = builder::SelectAndScatter(
          in, out_grad, zero, {"select", "scatter"}, window_dimensions,
          window_strides, paddings, in.GetType());

      return builder::Transpose(tmp_op, {0, 3, 1, 2});
    } else {
      return builder::SelectAndScatter(in, out_grad, zero,
                                       {"select", "scatter"}, window_dimensions,
                                       window_strides, paddings, in.GetType());
    }
  }
}