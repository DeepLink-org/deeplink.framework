...
from ..common.torch_deepcopy import deepcopy_to_fake_tensor_hf_hook_patched
...
class TestLlamaFinetune():
    def test_train(
        ...
        # model/data params
        backend: str,
        base_model: str = f"{models_dir}/base_model",  # the only required argument
        data_path: str = f"{models_dir}/alpaca_data.json",
        ...
        if data_path.endswith(".json") or data_path.endswith(".jsonl"):
            data = load_dataset("json", data_files=data_path, split='train[1%:2%]')
        else:
            data = load_dataset(data_path, split='train[1%:2%]')
        ...
        with extend_dynamo_plugin_backend(["topsgraph", "ascendgraph", "compile_profiler"]):
            trainer = transformers.Trainer(
                ...
                data_collator=transformers.DataCollatorForSeq2Seq(
                    tokenizer, pad_to_multiple_of=256, return_tensors="pt", padding=True
                ),
            )
