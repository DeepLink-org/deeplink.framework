...
need_node = ['add', 'sub', 'mul', 'div', 'view', 'scatter', 'full', 'lt',
             'where', 'convolution', 'le', 'eq', 'scalar_tensor', 'maximum',
...
class AscendCodegen(torch.fx.Interpreter):
    ...
    def get_attr(self, name, target, args, kwargs):
        ...
        op = getattr(self.override, 'get_attr')(self.args_dict[name], self.cur_node)
        self.common_nodes.append(op)
...
class AscendOverrides:
    ...
    def mul(name, node, x, y):
        ...
        if not isinstance(y_node, torch.fx.node.Node):
            ...
            broadcast_op.set_input("shape", f"{name}_shape")
            ops.extend([scalar_op.to_node(), shape_op.to_node(), broadcast_op.to_node()])
        ...
        if y_node.meta['val'].dtype != torch.complex64:
            ...
            if np.prod(x_shape) < np.prod(y_shape):
                x_shape_op = OP(f"{name}_x_shape", "Const")
                ...
                broadcast_op.set_input("shape", f"{name}_x_shape")

                ops.extend([x_shape_op.to_node(), broadcast_op.to_node()])
                x_name = f"{name}_x_broadcast_to"
            elif np.prod(x_shape) > np.prod(y_shape):
                y_shape_op = OP(f"{name}_y_shape", "Const")
                ...
                broadcast_op.set_input("shape", f"{name}_y_shape")

                ops.extend([y_shape_op.to_node(), broadcast_op.to_node()])
    ...
    @staticmethod
    def get_attr(name, x):
        x_shape = list(x.meta['val'].shape)
        x_value = x.meta['val'].tolist()
        if not isinstance(x_value, list):
            x_value = [x_value]

        torch_dtype = x.meta['val'].dtype
    ...
    def masked_fill(name, node, x, mask, value):
        ...
        broadcast_op.set_input("shape", f"{name}_shape")
        ops.extend([op.to_node(), shape_op.to_node(), broadcast_op.to_node()])
    ...
    def embedding(name, weight, indices, padding_idx):
        ...
        #op1 = OP(f"{name}_indices", "Squeeze")
        #op1.set_input("x", indices)
        #op1.set_attr_int("axis", 0)
        op1 = OP(f"{name}", "GatherV2")
        op1.set_input("x", weight)
        op1.set_input("indices", indices)
        op1.set_input("axis", f"{name}_axis")
        # op1.set_attr_int("batch_dims", 0)
        # op1.set_attr_int("dim", 0)

        #op2 = OP(f"{name}", "Squeeze")
        #op2.set_input("x", f"{name}_gather")
        #op2.set_attr_int("axis", 0)
    ...
    def div(name, node, x, y):
        ...
        if isinstance(y_node, torch.fx.node.Node):
            ...
            div_value = 1.0 / y_node
            return getattr(AscendOverrides, "mul")(name, node, x, div_value)
    ...
    def maximum(name, node, a, b):
        ...
        if a_node.meta['val'].dtype == torch.float16:
            ...
            cast_op = OP(b_name, "Cast")
            cast_op.set_input('x', b)
            cast_op.set_attr_int('dst_type', get_ascend_dtype_num("FLOAT16"))
        ...
        if cast_op is None:
            return op.to_node()
        return [cast_op.to_node(), op.to_node()]
    ...
    @staticmethod
    def full(name, node, dims, fill_value, device='xpu', pin_memory=False):
        if len(dims) == 0:
            dims = [1]
    ...
    def repeat_interleave(name, node, x, output_size):
        ...
        assert(int(output_size / numel) == 1)

        op = OP(f"{name}", "Identity")
        op.set_input("x", x)

        # TODO! fix implementation of repeatinterleave
        # Consider ConcatD for repeats > 1
    ...
    def arange(name, end, start=0, step=1, device='xpu', pin_memory=False):
        ...
        op.set_attr_tensor("value", "INT32", "INT32", "ND", [start], [])

        op1 = OP(f"{name}_end", "Const")
        op1.set_attr_tensor("value", "INT32", "INT32", "ND", [end], [])

        op2 = OP(f"{name}_step", "Const")
        op2.set_attr_tensor("value", "INT32", "INT32", "ND", [step], [])

        op3 = OP(name, "Range")
        op3.set_input("start", f"{name}_start")
        op3.set_input("limit", f"{name}_end")
        op3.set_input("delta", f"{name}_step")
    ...
    def cumsum(name, x, dim, dtype=None):
        ...
        op1.set_input("x", x)
        op1.set_input("axis", f"{name}_dim")
        op1.set_attr_bool("exclusive", False)
        op1.set_attr_bool("reverse", False)
        return [op.to_node(), op1.to_node()]
