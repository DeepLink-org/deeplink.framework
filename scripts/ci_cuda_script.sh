...
function config_dipu_cuda_cmake() {
...
function autogen_diopi_wrapper() {
    python scripts/autogen_diopi_wrapper/autogen_diopi_wrapper.py                   \
        ...
        --autocompare False                                                         \
        --diopi_adapter_header torch_dipu/csrc_dipu/vendor/cuda/diopi_adapter.hpp   \
...
function build_diopi_lib() {
    ...
    sh scripts/build_impl.sh cuda || exit -1
    cd -
}
...
function build_dipu_lib() {
    ...
    config_dipu_cuda_cmake 2>&1 | tee ./cmake_cuda.log
