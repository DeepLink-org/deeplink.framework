...
class TestGroupNorm(TestCase):
    ...
    def init_module(self, num_groups, num_channels, affine = False):
        self.num_groups = num_groups
        self.num_channels = num_channels
        self.cpu_module = torch.nn.GroupNorm(self.num_groups, self.num_channels, affine = affine)
        self.dipu_module = torch.nn.GroupNorm(self.num_groups, self.num_channels, affine = affine)
        self.dipu_module = self.dipu_module.cuda()

    def run_group_norm(self):
        x1 = self.x.clone().cpu()
    ...
    def test_group_norm_affine(self):
        ...
        self.init_module(3, 6, affine = True)
        self.run_group_norm()
        self.init_module(6, 6, affine = True)
        self.run_group_norm()
        self.init_module(1, 6, affine = True)
        self.run_group_norm()

    def test_group_norm_no_affine(self):
        self.x = torch.randn(20, 6, 10, 10)

        self.init_module(3, 6, affine = False)
        self.run_group_norm()
        self.init_module(6, 6, affine = False)
        self.run_group_norm()
        self.init_module(1, 6, affine = False)
        self.run_group_norm()
