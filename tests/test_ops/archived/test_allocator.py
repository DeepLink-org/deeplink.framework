...
if __name__=='__main__':
    max_allocate = 1 << 15
    p1 = Process(target = test_allocator, args = (max_allocate, 1, "BF", 0),)
    p1.start()
    p1.join()

    p2 = Process(target = test_allocator, args = (max_allocate, 1, "BS", 0),)
    p2.start()
    p2.join()
    ...
    max_allocate = 1 << 30
    p4 = Process(target = test_allocator, args = (max_allocate, 17919, "BF", 3, False),)
    p4.start()
    p4.join()
