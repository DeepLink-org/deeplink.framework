...
DISABLED_TORCH_TESTS_ANY = {
    ...
    'TestTensorDeviceOpsXLA': {
        'test_block_diag_scipy',  #FIXME: RuntimeError: Error while lowering: f32[1,6]{1,0} dipu::unselect, dim=1, start=2, end=2, stride=0
        'test_mean_64bit_indexing_dipu',  # protobuf limit exceeded
    },
...
DISABLED_TORCH_TESTS_DIPU = union_of_disabled_tests(
    [DISABLED_TORCH_TESTS_ANY, DISABLED_TORCH_TESTS_DIPU_ONLY])

DISABLED_TORCH_TESTS = {
    'DIPU': prepare_match_set(DISABLED_TORCH_TESTS_DIPU),
...
class DIPUTestBase(DeviceTypeTestBase):
  ...
  def assertEqual(self, x, y, *args, **kwargs):
    ...
    if gmode == 'text':
      if type(x) == torch.Tensor and xm.is_dipu_tensor(x):
        print(
            '\nTest Graph (x):\n{}'.format(
                torch_dipu._DIPUC._get_dipu_tensors_text([x])),
            file=sys.stderr)
      if type(y) == torch.Tensor and xm.is_dipu_tensor(y):
        print(
            '\nTest Graph (y):\n{}'.format(
                torch_dipu._DIPUC._get_dipu_tensors_text([y])),
            file=sys.stderr)
    elif gmode == 'hlo':
      if type(x) == torch.Tensor and xm.is_dipu_tensor(x):
        print(
            '\nTest Graph (x):\n{}'.format(
                torch_dipu._DIPUC._get_dipu_tensors_hlo([x])),
            file=sys.stderr)
      if type(y) == torch.Tensor and xm.is_dipu_tensor(y):
        print(
            '\nTest Graph (y):\n{}'.format(
                torch_dipu._DIPUC._get_dipu_tensors_hlo([y])),
            file=sys.stderr)
    elif gmode:
