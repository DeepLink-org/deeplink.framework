...
def gen_dipu_helper(backend_index: BackendIndex) -> List[str]:
    if backend_index.dispatch_key != DispatchKey.DIPU:
        return [""]
...
"""
                ...
                if f.manual_kernel_registration or self.skip_dispatcher_op_registration:
                    return None
                elif self.backend_index.dispatch_key == DispatchKey.DIPU:
                    metadata = self.backend_index.get_kernel(f)
                    if metadata is None:
                        return None
