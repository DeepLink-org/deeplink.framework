...
class AscendCompileJob(DeviceCompileJob):
    def __init__(self, source_code) -> None:
        ...
        self._key, self._input_path = write(
            source_code.strip(),
            "json",
            extra=cpp_compile_command("i", "o", vec_isa=picked_vec_isa) + 'local_rank' + str(self._local_rank),
        )
        ...
        from torch_dipu.dicp.vendor.AscendGraph.codegen import load_and_run
    ...
    def get_compile_result(self):
        ...
        from torch_dipu.dicp.vendor.AscendGraph.codegen.load_and_run import AscendModel
