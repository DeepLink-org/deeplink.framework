...
def functions_code_gen(fun_config):
    if 'interface' in fun_config:
        ...
        #fun_config['interface'] = diopi_interface
    print('config:',fun_config.get('interface', '') + ";")
    print('create:',create_call_diop_interface_code_from(fun_config['schema']) + ';')
    ...
    #for input in fun_config['ins']:
        if input.strip().endswith('?'):
            ...
            #input_process_code += f"\nauto {input}_diopiHandle = nullptr;\n"
