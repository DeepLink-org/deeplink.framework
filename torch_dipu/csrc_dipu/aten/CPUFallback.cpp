...
void cpu_fallback(const c10::OperatorHandle& op, torch::jit::Stack* stack) {
    ...
    for (const auto idx : c10::irange(arguments.size())) {
        ...
        if (log_fallback_detail) {
            std::cout << "cpu_fallback:\t" << op.schema().name() << "\t arguments[" << idx << "] :" << schema_args[idx].name()
                      << "\ttype:" << schema_args[idx].type() << "\tisTensor:" << ivalue.isTensor() << "\talias_info:" << schema_args[idx].alias_info()
                      << std::endl;
        }
    ...
    for (const auto i : c10::irange(tensor_args_indices.size())) {
        ...
        if ((alias_info != nullptr && alias_info->isWrite())) {
            ...
            if (log_fallback_detail) {
                std::cout << "force_copy_tensor: " << tensor_idx << ":" << cpu_tensors[i].options() << ",size:" << cpu_tensors[i].sizes() << ", "
                          << tensor_args[i].options() << ",size:" << cpu_tensors[i].sizes() << std::endl;
            }
            at::_copy_from_and_resize(cpu_tensors[i], tensor_args[i]);
        }
    }
