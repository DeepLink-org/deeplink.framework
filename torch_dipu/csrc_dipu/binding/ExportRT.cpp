...
static void registerDIPUDeviceProperties(py::module& m) {
    py::class_<DIPUDeviceProperties>(m, "_DIPUDeviceProperties")
        ...
        .def("__repr__", [](const DIPUDeviceProperties& prop) {
            ...
            stream << "_DIPUDeviceProperties(name='" << prop.name << "', major=" << prop.major << ", minor=" << prop.minor
                   << ", total_memory=" << prop.totalGlobalMem / kMega << "MB, multi_processor_count=" << prop.multiProcessorCount << ")";
            return stream.str();
        });
}
...
static void exportDevices(py::module& m) {
    ...
    m.def("_dipu_set_device", [](int idx) -> void { devproxy::setDevice(static_cast<devapis::deviceId_t>(idx)); });
    m.def("_dipu_get_device_count", []() -> int { return devproxy::getDeviceCount(); });
    m.def("_dipu_current_device", []() -> int { return static_cast<int>(devproxy::current_device()); });
    ...
    m.def(
        ...
        [](int device) -> DIPUDeviceProperties* { return dipu::device::getDevicePropertiesFromCache(device); },
...
static void exportStream(py::module& m) {
    ...
    pybind11::class_<DIPUStream>(m, "_DIPUStreamBase")
        .def(py::init([](int priority, c10::StreamId stream_id, c10::DeviceIndex device_index, int64_t device_type, uint64_t stream_ptr) {
                 if (stream_id || device_index || device_type) {
                     ...
                     return dipu::getStreamFromExternal(reinterpret_cast<deviceStream_t>(stream_ptr), devproxy::current_device());
                 } else {
             ...
             py::arg("stream_ptr") = 0)
        .def(py::init([](c10::DeviceIndex device_index, int isdefault) { return dipu::getCurrentDIPUStream(device_index); }))
        ...
        // cpp properties
        .def_property_readonly("stream_id", [](DIPUStream& stream) -> c10::StreamId { return stream.id(); })
        .def_property_readonly("device_index", &DIPUStream::device_index)
        .def_property_readonly("device_type", [](DIPUStream& stream) -> int64_t { return static_cast<int64_t>(stream.device().type()); })
        .def_property_readonly("dipu_stream", [](DIPUStream& stream) -> uint64_t { return (uint64_t)stream.rawstream(); })
        // use type_caster<at::Device>
        .def_property_readonly("device", [](DIPUStream& stream) -> at::Device { return stream.device(); });

    m.def(
        "_dipu_setStream",
        [](c10::StreamId stream_id, c10::DeviceIndex device_index) -> void { dipu::setCurrentDIPUStream(DIPUStream(device_index, stream_id)); },
        py::arg("stream_id") = 0,
        py::arg("device_index") = 0);

    m.def("_dipu_getCurrentStream", [](c10::DeviceIndex devIdx) -> DIPUStream { return dipu::getCurrentDIPUStream(devIdx); });
    m.def("_dipu_getDefaultStream", [](c10::DeviceIndex devIdx) -> DIPUStream { return dipu::getDefaultDIPUStream(devIdx); });
}
...
static void exportEvent(py::module& m) {
    ...
    pybind11::class_<DIPUEvent>(m, "_DIPUEventBase")
        // add flag in future
        .def(py::init([](bool enable_timing, bool blocking, bool interproces) { return DIPUEvent(); }),
        ...
        .def("record", pybind11::overload_cast<const DIPUStream&>(&DIPUEvent::record), "record event on stream")
        ...
        .def_property_readonly("dipu_event", [](DIPUEvent& self) { return (uint64_t)self.rawevent(); })
...
static void exportCommunicator(py::module& m) {
    pybind11::class_<ProcessGroupDICL, c10d::Backend, c10::intrusive_ptr<ProcessGroupDICL>>(m, "ProcessGroupDICL")
        .def(py::init([](const c10::intrusive_ptr<c10d::Store>& store, int rank, int size, const std::chrono::milliseconds& timeout) {
...
static void exportMemCaching(py::module& m) {
    ...
    m.def("memory_reserved", [](const c10::Device& device) -> size_t { return memoryReserved(device); });

    m.def("memory_allocated", [](const c10::Device& device) -> size_t { return memoryAllocated(device); });

    m.def("max_memory_reserved", [](const c10::Device& device) -> size_t { return maxMemoryReserved(device); });

    m.def("max_memory_allocated", [](const c10::Device& device) -> size_t { return maxMemoryAllocated(device); });
}
...
static void patchStorage(py::module& m) {
    ...
    m.def("storage_resize_", [](at::Storage stor, int64_t newsize) -> at::Storage {
        if (stor.device_type() != DIPU_DEVICE_TYPE) {
            TORCH_CHECK(false, "UntypedStorage.resize_: dipu storage resize not support other device type ", stor.device_type());
        } else {
...
static void patchTensor(py::module& m) {
    m.def("is_dipu", [](at::Tensor self) -> bool { return dipu::isDeviceTensor(self); });
}
