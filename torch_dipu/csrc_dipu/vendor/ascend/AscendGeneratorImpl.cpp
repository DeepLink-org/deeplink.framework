...
static deviceHandle_t getDeviceHandler(c10::DeviceIndex device_index) {
  ...
  DIPU_CALLACLRT(aclrtCreateContext(handle, device_index));
  DIPU_CALLACLRT(aclrtSetCurrentContext(handle));
...
public:
  NPUGeneratorImpl(at::DeviceIndex device_index): dipu::DIPUGeneratorImpl(device_index) {
  }
  /**
   * get_init_state_flag
   *
