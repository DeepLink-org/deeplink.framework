...
class TestIota():
    ...
    @pytest.mark.parametrize("end", [5, 9])
    @pytest.mark.parametrize("compiled_model", compiled_model)
    def test_torch_iota(self, length, start, end, compiled_model):
        output = model(length, start, end)
        dynamo.reset()
        update_dynamo_config(compiled_model.dynamic)
        dicp_output = compiled_model.model(length, start, end)
